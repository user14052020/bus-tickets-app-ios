...
public struct VAWeekDaysViewAppearance {
    ...
    public init(
        ...
        self.weekDayTextFont = UIFont(name:"HelveticaNeue-Bold",size:13)!
...
//            y: separatorY,
    ...
    private func setupView() {
        ...
        names.enumerated().forEach { index, name in
            ...
            if (name == "сб" ||  name == "вс")  {
                label.textColor = UIColor(red:143/255, green:143/255.0, blue:143/255, alpha:1.00)
            }else{
                label.textColor = UIColor(red:65/255, green:65/255.0, blue:65/255, alpha:1.00)
                
                
            }
            let labelAttributedText = NSMutableAttributedString(string: label.text!)
            labelAttributedText.addAttribute(NSAttributedString.Key.kern, value: CGFloat(1.08), range: NSRange(location: 0, length: labelAttributedText.length))
