...
class VAMonthView: UIView {
    ...
    private func setupMonthLabel() {
        ...
        var textLabel = monthLabel!.text!

        let correctTextLabel = " " + textLabel.prefix(1).uppercased() + textLabel.lowercased().dropFirst() + "          "

        monthLabel?.text = correctTextLabel
...
//        let size = CGSize(width: (monthLabel?.frame.width)! + 40, height: (monthLabel?.frame.height)!)
//        monthLabel?.frame.size = size
//        monthLabel?.frame = CGRect(origin: CGPoint(x: frame.width/2 - (monthLabel?.frame.width)!, y: 0), size: size)
        ...
        separatorView.backgroundColor = UIColor(red:244/255, green:244/255, blue:244/255, alpha:1)
        insertSubview(separatorView, belowSubview: monthLabel!)
